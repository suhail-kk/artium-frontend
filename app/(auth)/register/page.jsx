...
export default function Register() {
    ...
    const API_registerUser = useApi(register)
    ...
    const onSubmit = async () => {
        ...
        handleSubmit(async (params) => {
            ...
            const { isError, errors: apiErrors } =
                await API_registerUser.request(payloadData)

            if (isError) {
                errorSetter(apiErrors, setError)
            } else {
                success('Registration Successful, Thank you for joining us!')
                reset()
                ...
                <div className="md:w-1/2 pr-5 flex flex-col justify-between">
                    <div className='h-full content-center'>
                        ...
                        <form className="mt-6" action="#" method="POST">
                                ...
                                <Controller
                                    ...
                                    rules={{
                                        ...
                                        pattern: {
                                            value: /^[A-Z][a-zA-Z\s]*$/,
                                            message:
                                                'Full name must start with a capital letter and contain only letters and spaces.',
                                        },
                                        maxLength: {
                                            value: 80,
                                            message:
                                                'Full name must be less than 80 characters.',
                                        },
                                    }}
                                    render={({ field: { onChange, onBlur, value } }) => (
                                        <TextInput label="Full name" placeholder="Full name" onChange={onChange} value={value} error={errors.full_name} />
                                    )}
                                />
                            ...
                            <button disabled={API_registerUser.loading} onClick={handleSubmit(onSubmit)} type="submit" className="w-full block bg-blue-500 hover:bg-blue-primary focus:bg-blue-primary text-white font-semibold rounded-lg
                  px-4 py-3 mt-6">{API_registerUser.loading ? "Loading..." : "Register"}</button>
                        </form>
                    </div>
                    ...
                    <div className="text-sm flex justify-between items-center mt-3">
                        ...
                        <Link href='/login'>
                            <button className="py-2 px-5 ml-3 bg-white border rounded-xl hover:scale-110 duration-300 border-blue-400  ">Login</button></Link>
                    </div>
                </div>
